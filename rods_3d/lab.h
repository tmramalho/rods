...
class lab {
	...
	private:
		...
		int waitForEquilibrium(int eqTime);

		double integrateAcFunction(double *xsi, int tend);

		void saveAcFunction(int mode, double *xsi, int tend, double T);

		double calcAcFunction(int *vArray, int dt, double T, int mode = 0);
	...
	public:
		...
		void simCorr(int runtime, int eqTime, int numSamples);

		void simMeas(int eqTime, int numSamples);
